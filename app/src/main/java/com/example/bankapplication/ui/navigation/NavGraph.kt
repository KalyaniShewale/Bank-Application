...
fun AppNavGraph(navController: NavHostController) {
    NavHost(navController, startDestination = Screen.Welcome.route) {
        ...
        composable(Screen.Payments.route) {
            PaymentsScreen(
                onDomesticClick = {
                    navController.navigate(Screen.Payment.createRoute(TransferType.Domestic.value))
                },
                onInternationalClick = {
                    navController.navigate(Screen.Payment.createRoute(TransferType.International.value))
                }
            )
        }
        ...
        composable(
            ...
            val transferType = TransferType.fromString(typeArg)

            PaymentScreen(transferType = transferType)
        }
    }
}
